...
# test data
data = np.array([[-1,-1],[-1,0],[0,1],[1,1],[1,2],[1,1],[-1,-1],[-1,0],[0,1]])
...
previous_l = 0.0
previous_likelihood = np.zeros((N,K))


num_iter = 1
for t in range(3,N):
  print 'num iteration : ', t
  ...
  for k in range(K):
    likelihood[:,k] = [r * mv_norm(d, mu[k], np.array(sigma[k]))*pi_k[k] for d in data] + (1-r) * previous_likelihood[:,k]
  ...
  l = r * sum(map(np.log,[sum(likelihood[n,:]) for n in range(N)]))/N + (1 - r)*previous_l
